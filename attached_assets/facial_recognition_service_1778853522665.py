...
MIN_FACE_PX      = 35          # Minimum face side in pixels (lowered: catch faces at distance/angle)
VOTE_WINDOW      = 3           # Sliding window of votes considered for confirmation
VOTE_REQUIRED    = 1           # Votes needed to confirm (1 = first above-threshold match confirms; cosine threshold is the quality gate)
...
# DeepSORT parameters
DEEPSORT_MAX_AGE    = 30   # Tracks survive longer – walking creates brief occlusions
DEEPSORT_N_INIT     = 1    # Confirm tracks on first detection
DEEPSORT_MAX_IOU    = 0.75 # Slightly looser IOU association for fast-moving faces
...
# Face quality gating
# Laplacian variance < BLUR_THRESHOLD → crop is too blurry; skip embedding.
# Lowered from 50 → 30: walking faces naturally have variance 20–60; the old
# threshold discarded nearly every frame during a walkthrough.
BLUR_THRESHOLD        = 30.0
# During detected motion the gate is relaxed further so votes accumulate even
# when the subject is mid-stride.  Very blurry crops (< 12) still get dropped.
BLUR_THRESHOLD_MOTION = 12.0

# Instant confirmation: if a single embedding is this close to a registered
# person it is accepted immediately without waiting for the full vote window.
# Prevents a fast-walking person from clearing the frame before VOTE_WINDOW fills.
INSTANT_CONFIRM_DIST  = 0.28

# Extra fractional padding added to the YOLO face box when motion is detected.
# Walking creates slight misalignment between the predicted box and the actual
# face; padding ensures FaceNet receives a complete face crop.
WALK_CROP_PADDING     = 0.15
...
YOLO_FACE_CONF  = 0.35   # Detection confidence threshold – lowered from 0.40 to catch angled/moving faces
...
IDENTITY_CACHE_SECS      = 15.0  # Seconds to retain a departed identity (extended for walk-through re-entry)
IDENTITY_CACHE_THRESHOLD = 0.42  # Cosine distance for instant re-identification (wider: profile angles on re-entry)
...
# Force OpenCV to use TCP for RTSP and configure FFmpeg for resilience.
# fflags=+discardcorrupt : drop frames that fail H264 decoding instead of
#   passing a broken numpy array to Python (root cause of the get_frame crash).
# flags=+low_delay        : reduce buffering so bad frames are noticed faster.
# allowed_media_types=video : ignore stray audio RTP packets that cause bad-cseq warnings.
...
def build_face_embedder():
    ...
    # Check for a working C++ compiler before attempting torch.compile.
    # The inductor backend requires g++ or MSVC; without one it emits hundreds
    # of [WARNING] log lines from torch._dynamo.convert_frame before failing.
    # Probing here is instant and avoids triggering that logger entirely.
    _cxx_ok = False
    if hasattr(torch, "compile"):
        try:
            ...
            print("           Install Visual Studio Build Tools or MinGW-w64 to enable it.")
    ...
    if _cxx_ok:
        try:
            ...
            model       = compiled
            warmup_done = True
            print("  FaceNet: torch.compile enabled (dynamic mode). Warmup complete.")
        except Exception as exc:
            print(f"  FaceNet: torch.compile failed ({type(exc).__name__}) – running in eager mode.")
...
def is_blurry(crop: np.ndarray, threshold: float = BLUR_THRESHOLD) -> bool:
    """
    True when the Laplacian variance of the crop is below `threshold`.
    Pass BLUR_THRESHOLD_MOTION during high-motion frames to be more permissive.
    """
    if crop is None or crop.size == 0:
        return True
    ...
    def update(
        ...
        if to_recognize:
            ...
            for (tid, _), (emp_id, cos_dist) in zip(to_recognize, batch_results):
                ...
                # Instant confirm: a single very-confident match skips the vote
                # window entirely.  Critical for fast walkthroughs where the
                # subject may only be in the optimal zone for 1–2 frames.
                if cos_dist < INSTANT_CONFIRM_DIST and state["confirmed_id"] is None:
        ...
        while self._running[0]:
            ...
            if (
                ...
                try:
                    ...
                    _motion_active = self.motion_score > MOTION_THRESHOLD
                    proc_frame    = preprocess_frame(frame)
                    # Lower YOLO confidence during motion to catch angled/moving faces
                    # that score slightly below the static threshold.
                    yolo_conf    = max(0.25, YOLO_FACE_CONF - 0.08) if _motion_active else YOLO_FACE_CONF
                    ...
                    for c in candidates:
                        ...
                        w, h = c["w"], c["h"]
                        if _motion_active:
                            # Add padding during motion: YOLO bbox may clip face
                            # edges when the subject is mid-stride.
                            px = int(w * WALK_CROP_PADDING)
                            py = int(h * WALK_CROP_PADDING)
                    ...
                    # ── Stage 3: motion-adaptive blur gate ───────────────
                    # During motion use a lower threshold so that slightly blurry
                    # walking-frame crops are still embedded.  The old static 50.0
                    # threshold discarded nearly every crop during a walkthrough.
                    active_blur_thresh = BLUR_THRESHOLD_MOTION if _motion_active else BLUR_THRESHOLD
                    gated_crops = [
                        None if is_blurry(c, active_blur_thresh) else c
        ...
        while True:
            ...
            for worker in workers:
                try:
                    ...
                    print(f"[{worker.name}] Render error (skipping frame): {exc}")
